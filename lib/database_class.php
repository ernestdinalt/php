...
class DataBase {
	...
	public function getMaxField($table_name, $field) {
		$data = $this->select($table_name, array("*"), "", "$field", false, "1");
		return $data[0]["$field"];
	}
	
	public function getMinField($table_name, $field) {
	...
	public function getElementInRangeOfDates($table_name, $date_start_day, $date_start_month, $date_start_year, $date_end_day, $date_end_month, $date_end_year) {
		if(!checkdate($date_start_month, $date_start_day, $date_start_year)) return false;
		if(!checkdate($date_end_month, $date_end_day, $date_end_year)) return false;
		$date_start = mktime("00", "00", "00", "$date_start_month", "$date_start_day", "$date_start_year");
		$date_end = mktime("00", "00", "00", "$date_end_month", "$date_end_day", "$date_end_year");
		$array = $this->select($table_name, array("*"), "`date` BETWEEN $date_start and $date_end", "", true, "");
		return $array;
	}
